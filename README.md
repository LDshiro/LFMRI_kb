...
- 一般的な文献調査: `literature/search-strategies/` → `literature/reading-list.md` → `literature/references.bib` → `literature/paper-notes/template.md` → `literature/evidence-tables/` → `kb/`
...
6. 採用した論文の書誌情報を `literature/references.bib` に追加する。
7. 論文ノートを `literature/paper-notes/` に作成する。テーマ専用テンプレートがある場合はそれを優先する。
8. 抽出した主張・数値・限界を `literature/evidence-tables/` と `kb/` に再配置する。
9. `make check` と `make index` を実行してからPRまたは共有に進む。
...
reading list / references.bib
  ↓ candidate papers / citation metadata
