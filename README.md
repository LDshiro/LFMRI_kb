...
```text
kb/                 低磁場MRIのトピック別ナレッジ
literature/         文献リスト、論文ノート、検索式、source list、エビデンステーブル
...
- 一般的な文献調査:
  `literature/search-strategies/` -> `literature/reading-list.md` -> `literature/purchase-list.md` -> `literature/references.bib` -> `literature/paper-notes/template.md` -> `literature/evidence-tables/` -> `kb/`
- ハードウェア設計法調査:
  `literature/search-strategies/pubmed-hardware-design-methods.md`, `literature/search-strategies/scholar-hardware-design-methods.md`, `literature/paper-notes/hardware-design-method-template.md`, `literature/evidence-tables/hardware-design-methods.csv`, `kb/hardware/design-methods.md`
- 外部調査リスト取り込み:
  `literature/source-lists/template.md`, `literature/source-lists/README.md`, `scripts/import_external_source_list.py`

## 外部調査リストの扱い

- 外部で作成した論文リストは、まず `literature/source-lists/` に置きます。
- `literature/source-lists/*.md` を正本とし、registry CSV、paper notes、reading list、purchase list、KB は派生物として更新します。
- 推奨フォーマットは `literature/source-lists/template.md` に固定しています。
- 取り込みは `scripts/import_external_source_list.py` で行います。
  - Windows:
    `py -3 scripts/import_external_source_list.py --input literature/source-lists/<source-list>.md --access-audit literature/source-lists/<source-list>-access-audit.csv`
  - macOS / Linux:
    `python3 scripts/import_external_source_list.py --input literature/source-lists/<source-list>.md --access-audit literature/source-lists/<source-list>-access-audit.csv`
- access audit は時間とともに変わるため、`*-access-audit.csv` は再確認前提で管理します。
...
8. 外部調査リストから始める場合は、原本を `literature/source-lists/` に置き、import script で派生物を生成する。
9. 論文ノートを `literature/paper-notes/` に作成または更新する。テーマ専用テンプレートがある場合はそれを優先し、imported note は `draft` の intake note として扱う。
10. 抽出した主張、数値、限界を `literature/evidence-tables/` と `kb/` に再配置する。
11. `make check` と `make index` を実行してからPRまたは共有に進む。
...
external curated source list
  -> canonical source file under literature/source-lists/
import script
  -> registry csv / imported paper notes / reading list / purchase list
